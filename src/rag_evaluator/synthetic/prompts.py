...
from rag_evaluator.question_types.registry import get_question_type_rules
...
def build_chunk_block(chunks: list[Chunk]) -> str:
    """
    Render source chunks into a deterministic prompt block.
    :param chunks:
    :return:
    """
    sections: list[str] = []
...
def _build_type_specific_requirements(
    question_types: list[QuestionType],
) -> list[str]:
    rules_by_type = get_question_type_rules(question_types)
    instructions: list[str] = []

    for question_type in question_types:
        rule = rules_by_type[question_type]
        instructions.extend(f"- {instruction}" for instruction in rule.prompt_instructions())
...
def build_synthetic_generation_prompt(
    ...
    *,
    ...
    sample_count = (
        ...
        else "as many high-quality samples as appropriate"
    )

    type_specific_requirements = _build_type_specific_requirements(allowed_types)
